...
async def async_weather(args):
    ...
    try:
        ...
        pass
    try:
        content += ":cloud_rain: Volume de pluie (dernières 3h) : " + \
            str(data_weather["rain"]["3h"]) + "L/m²\n"
    except:
        pass
    try:
        content += ":cloud_snow: Volume de neige (dernières 3h) : " + \
            str(data_weather["snow"]["3h"]) + "L/m²\n"
    except:
        pass
    temp_celcius = str(round(data_weather["main"]["temp"] - 273.15, 1))
    content += f":thermometer: Température : {temp_celcius}°C\n"
    content += ":sweat_drops: Humidité : " + \
        str(data_weather["main"]["humidity"]) + "%\n"
    ...
    embed = discord.Embed(title="Météo actuelle à " + data_weather["name"] + " :flag_" + str(
    ...
    embed.set_footer(text=f"Date locale : {current_time}",
            ...
            if member.activity:
                ...
                if "counter" in member.activity.name.lower():
                    role = discord.utils.get(
                        member.guild.roles, id=autorole_server.split("/")[0])
                elif "payday" in member.activity.name.lower():
                    role = discord.utils.get(
                        member.guild.roles, id=autorole_server.split("/")[1])
                elif "minecraft" in member.activity.name.lower():
                    role = discord.utils.get(
                        member.guild.roles, id=autorole_server.split("/")[2])
                elif "rocket" in member.activity.name.lower():
                    role = discord.utils.get(
                        member.guild.roles, id=autorole_server.split("/")[3])
                elif "fortnite" in member.activity.name.lower():
                    role = discord.utils.get(
                        member.guild.roles, id=autorole_server.split("/")[4])
                elif "overwatch" in member.activity.name.lower():
                    role = discord.utils.get(
                        member.guild.roles, id=autorole_server.split("/")[5])
                else:
                    role = None
                print(role)
                if not role:
                    ...
                    channel = [x.id for x in member.guild.channels if str(
                        x.name) == "bots"][0]
...
async def help(ctx, *, args=None):
    ...
    if not ctx.message.author.bot == True:
        commands_type = ["infos_", "utilities_", "stats_",
                         "feedback_", "admin_configs_", "admin_rights_"]
        commands_type_special = ["flc_", "theswe_", "ism_"]
        commands_type_special_id = [
            "371687157817016331", "199189022894063627", "391272643229384705"]
        if not args:
            commands_type_emoji = ["informations", "utilities", "statistics", "feedback",
                                   "admin_configs", "admin_rights", "serv_flc", "serv_theswe", "serv_ism"]
            commands_title = [translations[x]
                              for x in translations.keys() if "msg_title_" in x]
            embed = discord.Embed(
                title=translations["msg_list_commands"], url=translations["url_website_discord"], color=0x00ff00)
            for cmds_type in range(len(commands_type)):
                embed_type = [x for x in translations.keys(
                ) if commands_type[cmds_type] in x]
                embed_var = ""
                ...
                embed.add_field(name=emo(
                    commands_type_emoji[cmds_type]) + commands_title[cmds_type], value=embed_var, inline=True)
            for cmds_type in range(len(commands_type_special)):
                embed_type = [x for x in translations.keys(
                ) if commands_type_special[cmds_type] in x]
                embed_var = ""
                if str(ctx.message.guild.id) == commands_type_special_id[cmds_type]:
                    ...
                    embed.add_field(name=emo(
                        commands_type_emoji[index]) + commands_title[index], value=embed_var, inline=True)
            if ctx.message.author.id == 246943045105221633:  # Quentium user ID
                ...
                embed.add_field(name=emo("user_quentium") +
                                commands_title[-1], value=embed_var, inline=True)
            donation = translations["msg_donation"].format(
                "https://www.paypal.me/QLienhardt")
            embed.add_field(
                name=translations["msg_caption"], value=translations["msg_caption_desc"], inline=True)
            embed.add_field(
                name=translations["msg_warning"], value=translations["msg_warning_desc"] + donation, inline=True)
            embed.set_footer(
                text=translations["msg_more_infos"], icon_url=translations["url_logo_bot"])
        else:
            all_commands = []
            commands_type += commands_type_special
            commands_keys = [x for x in translations.keys() if any(
                [y in x for y in commands_type])]
            for types in commands_type:
                all_commands += [x.split(types)[1]
                                 for x in translations.keys() if types in x]
            for types_aliases in commands_keys:
                all_commands += [x for x in translations[types_aliases]
                                 ["aliases"].split(" / ") if not x == ""]

            if any(x.lower() == args.lower() for x in all_commands):
                if any([x for x in commands_keys if args in translations[x]["aliases"]]):
                    command_type = [y for y in commands_keys if any(
                        [args in x for x in translations[y]["aliases"].split(" / ")])][0]
                else:
                ...
                command_aliases = [command_type.split(
                    "_")[1]] + translations[command_type]["aliases"].split(" / ")
                desc_text = f"```{args}```\n**{command_desc}**\n\n"
                ...
                embed = discord.Embed(
                    title=None, description=desc_text, color=0x03A678)
            else:
...
async def help_old(ctx):
    ...
    :warning: Si vous rencontrez un problème, merci de la soumettre avec la commande `+bug` ou en rejoignant le serveur de test : https://discord.gg/5sehgXx\n"""
        ...
        embed.add_field(name=":video_game: Commandes **UTILISATEUR** :",
        ...
        if ctx.message.author.id == 246943045105221633:
            embed.add_field(name=":eyes: Commandes **QUENTIUM** :",
                            value=commands_quentium, inline=True)
...
async def listinvites(ctx):
    ...
    if not ctx.message.author.bot == True:
        ...
        embed = discord.Embed(
            title=translations["msg_invites"], description="- " + content, color=0x00FFFF)
        return await ctx.send(embed=embed)
...
async def listbans(ctx):
    ...
    if not ctx.message.author.bot == True:
        ...
        if list_banned:
            ...
            content = translations["msg_any"]
        embed = discord.Embed(
            title=translations["msg_invites"], description="- " + content, color=0xFF0000)
        return await ctx.send(embed=embed)
...
async def msgtotal(ctx, *args):
    ...
    if not ctx.message.author.bot == True:
        if not ctx.message.channel.guild.me.guild_permissions.administrator:
            return await ctx.send(translations[lang_server]["GLOBAL"]["msg_bot_admin"])
        if len(args) == 2:
        ...
        counter = 0
        embed = discord.Embed(
            title=translations["msg_calculating"], color=0xFFA500)
        ...
        if msg_total == True:
            embed.add_field(name=translations[lang_server]["GLOBAL"]["dashes"], value="**" + str(member) +
        ...
        embed.set_footer(
            text=f"{translations[lang_server]['GLOBAL']['asked_by']}", icon_url=ctx.message.author.avatar_url)
        if not isinstance(ctx.channel, discord.TextChannel):
...
async def userstats(ctx, *, member: discord.Member = None):
    ...
    if not ctx.message.author.bot == True:
        ...
        user_game = "Aucun" if member.activity == None else str(
        ...
        embed.add_field(name="Statistiques de __***" + user_name + "***__",
...
async def serverstats(ctx):
    ...
    if not ctx.message.author.bot == True:
        ...
        serv_afk_channel = "Aucun" if serv.afk_channel == None else str(
            serv.afk_channel)
        ...
        if len(serv.icon_url):
            ...
            embed.set_thumbnail(
                url="https://quentium.fr/+Files/discord/question.png")
        content = "```autohotkey\n" \
            "Nom:                %s\n" \
            "ID:                 %s\n" \
            "Propriétaire:       %s (%s)\n" \
        ...
        embed.add_field(name="Statistiques du serveur __***" + serv_name + "***__",
    ...
    if not ctx.message.author.bot == True:
        ...
        if any(x in args.split()[0].lower() for x in ["list", "liste", "remove", "delete", "clear", "removeall", "deleteall", "clearall"]):
            if any(x == args.split()[0].lower() for x in ["list", "liste"]):
                ...
                if any(x.lower() == remove for x in triggers[str(server_id)].keys()):
                    ...
                    embed = discord.Embed(title="Réaction supprimée :",
                                          description=f"**{remove}**", color=0xBFFF00)
                else:
                    embed = discord.Embed(
        ...
        if not '"' in args and not "'" in args:
            if len(args.split()) == 2:
                ...
                embed = discord.Embed(
                    title="Il y a pas assez d'arguments pour créer la réaction, ajoutez-en entre des guillements pour délimiter votre message.", color=0xBFFF00)
                return await ctx.send(embed=embed)
            else:
                embed = discord.Embed(
                    title="Il y a trop d'arguments pour créer la réaction, mettez des guillements pour délimiter votre message.", color=0xBFFF00)
                return await ctx.send(embed=embed)
        else:
        ...
        if not any(x == str(server_id) for x in triggers.keys()):
            ...
            if trigger.lower() in triggers[str(server_id)].keys():
                embed = discord.Embed(
                    title="Il y a déjà un déclencheur pour ce message. Supprimez le puis refaites la commande.", color=0xBFFF00)
                return await ctx.send(embed=embed)
        triggers[str(server_id)][trigger.lower()] = response
        with open("extra/triggers.json", "w", encoding="utf-8", errors="ignore") as file:
            json.dump(triggers, file, indent=4)
                ...
                if "random" in number:
                    ...
                    while len(random_numbers) < nb_times:
                        random_numbers.append(
                            random.randint(1, len(channel_list)))
...
async def ban(ctx, *, member: discord.Member = None):
    ...
    if not ctx.message.author.bot == True:
        ...
        await member.ban()
        embed = discord.Embed(
            description="**%s** à été bannis !" % member.name, color=0xFF0000)
        return await ctx.send(embed=embed)
...
async def minecraft(ctx, *, omg_id=None):
    ...
    if not ctx.message.author.bot == True:
        ...
        draw.text(((W - w) / 2, 20), "Statistiques du Serveur",
                  font=font1, fill=(0, 255, 0))
        ...
        if list_players:
            ...
            draw.text((200, 300), "Serveur vide !",
